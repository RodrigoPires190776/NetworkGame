...
{
    public class DijkstraRouteDiscovery : BaseRouteDiscovery
    {
        public DijkstraRouteDiscovery(Network network, Dictionary<string, Property> properties)
            ...
            foreach (var node in Network.RouterIDList)
            {
                foreach (var nodeDestination in Network.RouterIDList)
                {
                    ...
                    foreach (var link in Network.Routers[node].Links.Values)
                    {
                        ...
                        nodeLinkValue.Add(link.ID, costSum / (allNodeDistances[GetOtherRouter(node, link)][nodeDestination] + link.LinkLength));
