...
{
    public class AveragePacketQueueTimeCombined : LineChartCombined
    {
        ...
        public AveragePacketQueueTimeCombined(List<Guid> networks, List<Game> game) :
           base("Average Packet Queue Time Combined", game)
        {
            Networks = networks;
        ...
        protected override void SaveData(List<UpdatedState> states)
        {
            ...
            foreach (var state in states)
            {
                ...
                {
                    timeSum += router.NumberPacketsInQueue;
                }
                listValues.Add((double)timeSum / state.UpdatedRouters.Count);

                var value = (double)timeSum / state.UpdatedRouters.Count;
